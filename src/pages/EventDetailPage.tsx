  ...
  useEffect(() => {
    const loadEvent = async () => {
      ...
      try {
        ...
        setError(t('eventDetail.errorLoadingEvent'));
        console.error(err);
      } finally {
        setLoading(false);
      }
    ...
    return (
      <div className="min-h-screen bg-gray-50 dark:bg-gray-900 flex items-center justify-center">
        <div className="text-center">
          <p className="text-red-600 dark:text-red-400 mb-4">{error || t('eventDetail.eventNotFound')}</p>
          <button
            onClick={() => navigate('/')}
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 py-8">
      ...
      <div className="max-w-4xl mx-auto px-4">
        <div className="mb-6">
          ...
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6 mb-6">
            ...
            <div className="space-y-2 text-sm text-gray-600 dark:text-gray-400">
              <p>
                <span className="font-medium">{t('eventDetail.accessCode')}</span>{' '}
                <span className="font-mono bg-gray-100 dark:bg-gray-700 px-2 py-1 rounded">{event.accessCode}</span>
              </p>
              <p>
                <span className="font-medium">{t('eventDetail.members')}</span> {event.members.join(', ')}
              </p>
              {event.hasFund && event.fundContributions && (
                <div className="mt-4 p-3 bg-blue-50 dark:bg-blue-900/30 rounded-md">
                  <p className="font-medium text-blue-800 dark:text-blue-300 mb-2">{t('eventDetail.commonFund')}</p>
                  ...
                  <p className="mt-2 font-semibold text-blue-600 dark:text-blue-400">
                    {t('eventDetail.totalFund')} {formatCurrency(
        ...
        <div className="flex gap-4 mb-6 flex-wrap">
          ...
          >
            {showExpenseForm ? t('eventDetail.hideForm') : t('eventDetail.addExpense')}
          </button>
          <button
            onClick={() => {
          ...
          >
            {showQRManager ? t('eventDetail.hideQRCodes') : t('eventDetail.manageQRCode')}
          </button>
          <button
            onClick={() => {
          ...
          >
            {showEditMembers ? t('eventDetail.hideEdit') : t('eventDetail.editMembers')}
          </button>
          <button
